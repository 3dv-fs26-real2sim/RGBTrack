"""
FoundationPose tracking using pre-scaled depth maps — no calibration.

Depth maps are loaded as-is (uint16 PNG mm → float32 m).
No RANSAC, no sim GT, no scaling applied.

Usage:
    python run_demo_raw_depth.py \
...
if __name__ == "__main__":
    ...
    for i in range(min(num_frames, len(reader.color_files))):
        ...
        if i == 0:
            pose = est.register(reader.K, color, depth, mask, args.est_refine_iter)
