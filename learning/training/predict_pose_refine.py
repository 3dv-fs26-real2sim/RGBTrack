...
def make_crop_data_batch(render_size, ob_in_cams, mesh, rgb, depth, K, crop_ratio, xyz_map, normal_map=None, mesh_diameter=None, cfg=None, glctx=None, mesh_tensors=None, dataset=None):
    ...
    # Compute crop window transformations

    tf_to_crops = compute_crop_window_tf_batch(
        pts=mesh.vertices, H=H, W=W, poses=ob_in_cams, K=K,
    ...
    # Warp perspective transformations
    ...
    pose_data = BatchPoseData(
        rgbAs=rgbAs, rgbBs=rgbBs, depthAs=None, depthBs=None,
        normalAs=normalAs, normalBs=normalBs, poseA=poseA, poseB=None,
        xyz_mapAs=xyz_mapAs, xyz_mapBs=xyz_mapBs, tf_to_crops=tf_to_crops,
        Ks=Ks, mesh_diameters=mesh_diameters
    )
...
@torch.inference_mode()
def make_crop_data_batch_depths(render_size, ob_in_cams, mesh, rgb, depths, K, crop_ratio, xyz_maps, normal_map=None, mesh_diameter=None, cfg=None, glctx=None, mesh_tensors=None, dataset:PoseRefinePairH5Dataset=None):
    ...
    xyz_mapBs = kornia.geometry.transform.warp_perspective(
        torch.as_tensor(xyz_maps, device='cuda', dtype=torch.float).permute(0,3,1,2), tf_to_crops, dsize=render_size, mode='nearest', align_corners=False)
...
class PoseRefinePredictor:
    def __init__(self,):
        ...
        if USE_TRT:
            self.model=RefineNetTrt("/mnt/ssd_990/teng/ycb/fp/FoundationPose/learning/models/refinenet.trt")
            self.model2=RefineNetTrt("/mnt/ssd_990/teng/ycb/fp/FoundationPose/learning/models/refinenet.trt")
        else:
    ...
    def predict(self, rgb, depth, K, ob_in_cams, xyz_map, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, iteration=5):
        ...
        for iter_idx in range(iteration):
            ...
            for b in range(0, pose_data.rgbAs.shape[0], bs):
                ...
                if USE_TRT:
                    if A.shape[0]==1:
                        output=self.model2.run_inference(A,B)
                    else:
                        output=self.model.run_inference(A,B)
                else:
    ...
    @torch.inference_mode()
    def predict_depth_batches(self, rgb, depths, K, ob_in_cams, xyz_map, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, iteration=5):
        '''
        @rgb: np array (H,W,3)
        @ob_in_cams: np array (N,4,4)
        '''
        ...
        if mesh_tensors is None:
            mesh_tensors = make_mesh_tensors(mesh_centered)
        rgb_tensor = torch.as_tensor(rgb, device='cuda', dtype=torch.float)
        depth_tensor = torch.as_tensor(depths, device='cuda', dtype=torch.float)
        xyz_map_tensor = torch.as_tensor(xyz_map, device='cuda', dtype=torch.float)
        trans_normalizer = self.cfg['trans_normalizer']
        if not isinstance(trans_normalizer, float):
            trans_normalizer = torch.as_tensor(list(trans_normalizer), device='cuda', dtype=torch.float).reshape(1,3)
            ...
            for b in range(0, pose_data.rgbAs.shape[0], bs):
                ...
                if USE_TRT:
                    ...
                    with torch.cuda.amp.autocast(enabled=self.amp):
                        output = self.model(A,B)
                for k in output:
                    output[k] = torch.tensor(output[k]).float()
        ...
        if get_vis:
            ...
            canvas = []
            padding = 2
            pose_data = make_crop_data_batch_depths(self.cfg.input_resize, torch.as_tensor(ob_centered_in_cams), mesh_centered, rgb, depths, K, crop_ratio=crop_ratio, normal_map=normal_map, xyz_map=xyz_map_tensor, cfg=self.cfg, glctx=glctx, mesh_tensors=mesh_tensors, dataset=self.dataset, mesh_diameter=mesh_diameter)
            ...
            pose_data = make_crop_data_batch_depths(self.cfg.input_resize, B_in_cams, mesh_centered, rgb, depths, K, crop_ratio=crop_ratio, normal_map=normal_map, xyz_map=xyz_map_tensor, cfg=self.cfg, glctx=glctx, mesh_tensors=mesh_tensors, dataset=self.dataset, mesh_diameter=mesh_diameter)
            canvas_refined = []
            for id in range(0, len(B_in_cams)):
                rgbA_vis = (pose_data.rgbAs[id]*255).permute(1,2,0).data.cpu().numpy()
                rgbB_vis = (pose_data.rgbBs[id]*255).permute(1,2,0).data.cpu().numpy()
