...
@torch.no_grad()
def make_crop_data_batch_depths(
    render_size,
    ob_in_cams,
    mesh,
    rgb,
    ...
    logging.info("Welcome make_crop_data_batch")
    B, H, W = depths.shape
    logging.info(f"depths.shape: {depths.shape}")
    args = []
    method = "box_3d"
    tf_to_crops = compute_crop_window_tf_batch(
        pts=mesh.vertices,
    ...
    # (B, 1, H, W)
    depthBs = kornia.geometry.transform.warp_perspective(
        torch.as_tensor(depths, dtype=torch.float, device="cuda")[None],
        tf_to_crops,
        dsize=render_size,
        mode="nearest",
        align_corners=False,
    )
    logging.info(f"depthBs.shape: {depthBs.shape}")
    assert depthBs.shape == rgbBs.shape
    if rgb_rs.shape[-2:] != cfg["input_resize"]:
        rgbAs = kornia.geometry.transform.warp_perspective(
            rgb_rs, tf_to_crops, dsize=render_size, mode="bilinear", align_corners=False
        )
...
class ScorePredictor:
    def __init__(self, amp=True):
        ...
        if USE_TRT==False:
            ...
            self.model = ScoreNetTrt(
                "/mnt/ssd_990/teng/ycb/fp/FoundationPose/learning/models/scorenet.trt"
            )
    ...
    def predict(
        ...
        if type(ob_in_cams)!=torch.Tensor:
        ...
        def find_best_among_pairs(pose_data: BatchPoseData):
            ...
            for b in range(0, pose_data.rgbAs.shape[0], bs):
                ...
                if pose_data.normalAs is not None:
                    A = torch.cat([A, pose_data.normalAs.cuda()], dim=1)
                    B = torch.cat([B, pose_data.normalBs.cuda()], dim=1)
                if USE_TRT==False:
    ...
    def predict_depth_batches(
        ...
        rgb = torch.as_tensor(rgb, device="cuda", dtype=torch.float)
        depths = torch.as_tensor(depths, device="cuda", dtype=torch.float)

        pose_data = make_crop_data_batch_depths(
            self.cfg.input_resize,
            ob_in_cams,
            mesh,
            rgb,
        ...
        def find_best_among_pairs(pose_data: BatchPoseData):
            ...
            for b in range(0, pose_data.rgbAs.shape[0], bs):
                ...
                if USE_TRT==False:
                    with torch.cuda.amp.autocast(enabled=self.amp):
                        print(f"A.shape: {A.shape}, B.shape: {B.shape}, L: {len(A)}")
                        output = self.model(A, B)
                else:
                    output = self.model.run_inference(A, B)
